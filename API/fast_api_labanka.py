...
@app.get("/predict")
def predict(with_previous: int,
            CNT_CHILDREN: int,               # 3
            AMT_INCOME_TOTAL: float,           # 50000
            DAYS_BIRTH: int,                 # 2530
            DAYS_EMPLOYED: int,              # 270
            ORGANIZATION_TYPE_TEXT: str,     # Electricity (Label encoding)
            EXT_SOURCE2: float,                # floating € [0,1]
            EXT_SOURCE3: float,                # floating € [0,1]
            DEF_60_CNT_SOCIAL_CIRCLE: int,   # Default (integer number)
            AMT_REQ_CREDIT_BUREAU_DAY: int,  # Default (integer number)
            AMT_REQ_CREDIT_BUREAU_YEAR: int, # Default (integer number)
            NAME_INCOME_TYPE: str,           # text, OHE
            CURR_AMT_ANNUITY: float,           # Only for current application, floating 3500.9
            CURR_AMT_CREDIT: float,            # Only for current applications floating 10000.5
            OCCUPATION_TYPE: str,            # text OHE
            NAME_CONTRACT_TYPE: str,         # if with_previous =1, value from params, if not default (OHE)
            NAME_PORTFOLIO: str,             # if with_previous =1, value from params, if not default (OHE)
            NAME_CONTRACT_STATUS: str,       # if with_previous =1, value from params, if not default (OHE)
            AMT_ANNUITY: float,                # if with_previous =1, value from params, if not default 
            AMT_CREDIT: float,                 # if with_previous =1, value from params, if not default 
            APPLICATION_CREDIT_DIF: float,     # if with_previous =1, value from params, if not default 
            PREV_REFUSED_COUNT: int,         # if with_previous =1, value from params, if not default 
            PREV_APPROVED_COUNT: int,        # if with_previous =1, value from params, if not default 
            AMT_PAYMENT: float,                # if with_previous =1, value from params, if not default (OHE)
            LATE_PAY: int,                   # if with_previous =1, value from params, if not default (OHE)
            AMT_MISS: int):
...
#Pending to calculate: ANNUITY_OVER_CREDIT_RATIO, CREDIT_TO_ANNUITY_RATIO (prev)
    ...
    if int(AMT_ANNUITY) == 0 or  int(AMT_CREDIT) == 0:
        CREDIT_TO_ANNUITY_RATIO = 0
    else:
        CREDIT_TO_ANNUITY_RATIO = int(AMT_ANNUITY) / int(AMT_CREDIT)
   ...
   'LATE_PAY', 'AMT_MISS', 'with_previous']
    ...
    cols = clean_df.select_dtypes(exclude=['float']).columns
    clean_df[cols] = clean_df[cols].apply(pd.to_numeric, downcast='float', errors='coerce')
